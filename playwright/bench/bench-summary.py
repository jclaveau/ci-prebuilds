#!/usr/bin/env python3
"""Write the benchmark step summary from the run's job timings and the act artifacts.

Runs in the summary job after every matrix cell; reads jobs.ndjson (the run's
jobs API dump), the downloaded artifacts in the working directory, BENCH_RUNS,
and appends the HTML tables to GITHUB_STEP_SUMMARY. It lived inline in
benchmark-playwright.yml until the run block outgrew GitHub's 21000-character
expression limit.
"""
...
for j in jobs:
    ...
    vals = {
        ...
        "perf": dur(perf.get("started_at"), perf.get("completed_at")),
...
def perf_delta(sc, ref, m, base):
    # The probe is the same work on every row, so its Δ % vs the reference is the
    # one column here that reads as browser speed rather than install plumbing.
    if sc == ref or m is None or not base:
        return "—", None
    p = (m - base) / base * 100
    return f"{'−' if p < 0 else '+'}{abs(p):.0f}%", p
...
def table(mode, order_list, ref, title):
    ...
    perf_base = mean_of(agg[(ref, mode)]["perf"])
    rows = []
    for sc in order_list:
        ...
        pp_str, pp_num = perf_delta(sc, ref, perf_m, perf_base)
        ok, n = oks[key]
        rows.append({
            "sc": sc,
            ...
            "d_num": d_num, "p_num": p_num, "pp_num": pp_num,
            "cell_str": cell_str, "d": d_str, "p": p_str, "pp": pp_str, "ok": ok, "n": n,
        })
    R = {k: col_range([r[k] for r in rows]) for k in ("d_num", "p_num", "pp_num")}
    out = [
        f"### {title}",
        "",
        '<table>',
        ...
        '<th>Test run</th><th>perf</th><th>Total</th>'
        '<th>Δ</th>'
        '<th>Δ %</th>'
        '<th>Δ perf %</th>'
        '</tr></thead>',
    ...
    for r in rows:
        out.append(
            ...
            f'<td>{badge(r["pp_num"], *R["pp_num"], r["pp"])}</td>'
            f'</tr>'
        )
...
for af in sorted(glob.glob("act-results/*.json")):
    ...
    key = (sc, mode)
    act_seen.add(key)
    for k in ("pull", "install", "ttt", "test", "perf", "total", "install_dl_mb", "disk_mb"):
...
def act_table(mode, order_list, ref, title):
    ...
    for sc in order_list:
        ...
        perf_m = mean_of(act[key]["perf"])
        pp, pp_num = perf_delta(sc, ref, perf_m, perf_base)
        if sc == ref:
            d, p, wd, wp, du = "—", "—", "—", "—", "—"
        ...
        rows.append({
            ...
            "du_num": du_num, "pp_num": pp_num,
            "cell_str": cell_str, "d": d, "p": p, "wd": wd, "wp": wp, "du": du, "pp": pp,
            "ok": ok, "n": n, "stub": False,
        })
    R = {k: col_range([r[k] for r in rows]) for k in
         ("d_num", "p_num", "wd_num", "wp_num", "du_num", "pp_num")}
    out = [
        f"### {title}",
        "",
        '<table>',
...
parts = [
    ...
    "**Test run** navigates a page served from the checkout itself (`playwright/tests/fixtures`), "
    "so it carries no DNS, TLS or CDN time. **perf** is the wall time of "
    "`playwright/bench/runtime-probe.cjs` on the cell's browsers — the same probe "
    "test-and-publish gates on — and **Δ perf %** compares it to the table's reference row; "
    "the per-row numbers are in the `perf-*` artifacts. "
