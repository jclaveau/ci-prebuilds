...
declare -A VARIANT=(
  ...
  [vcall]="-fsanitize=cfi-vcall -fsanitize-trap=cfi -fsanitize-ignorelist=../../tools/cfi/ignores.txt"
  [icall]="-fsanitize=cfi-icall -fsanitize-trap=cfi -fsanitize-ignorelist=../../tools/cfi/ignores.txt"
  [cfigen]="-fsanitize=cfi-vcall -fsanitize=cfi-icall -fsanitize-trap=cfi -fsanitize-ignorelist=../../tools/cfi/ignores.txt -fsanitize-cfi-icall-generalize-pointers"
)
: > /tmp/objs.txt
for src in $TUS; do
...
for tag in base cfi vcall icall cfigen; do
...
for tag in cfi vcall icall cfigen; do
...
for tag in base cfi vcall icall cfigen; do grep -m2 ' error: ' "$OUT/pgo-$tag.log" | sed "s/^/  $tag: /" | tee -a "$OUT/summary.txt"; done
echo "### DONE"
