...
- [chromium launch = the DSO closure, then the consumer image itself](project_chromium_launch_dso_closure.md) — trim shipped (0.79x); REVERSED 2026-09-18: base arm shows consumer image alone still pays launch 1.27x (scratch hits parity); Vulkan ICD ~4 ms; the rest is ICU walking 600 tzdata files against a missing /etc/localtime (strace: +9,300 file syscalls/launch), fix = symlink, PR #266
